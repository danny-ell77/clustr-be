...
class MemberVisitorViewSet(ModelViewSet):
    ...
    def revoke_invitation(self, request, pk=None):
        ...
        # Only allow revoking if the visitor hasn't checked in yet
        if visitor.status not in [Visitor.Status.CHECKED_IN, Visitor.Status.CHECKED_OUT]:
            ...
            if serializer.is_valid():
                serializer.save()
                return Response({'status': 'invitation revoked'}, status=status.HTTP_200_OK)
...
class MemberVisitorLogViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        """
        visitor_ids = Visitor.objects.filter(invited_by=self.request.user.id).values_list('id', flat=True)
