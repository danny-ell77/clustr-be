...
class VisitorEmailContext(BaseEmailContext):
    ...
    def __post_init__(self):
        ...
        if self.arrival_time and not self.formatted_arrival_time:
            self.formatted_arrival_time = self.arrival_time.strftime(
                "%H:%M on %B %d, %Y"
            )
        if self.departure_time and not self.formatted_departure_time:
            self.formatted_departure_time = self.departure_time.strftime(
                "%H:%M on %B %d, %Y"
            )
...
class PaymentReceiptEmailContext(BaseEmailContext):
    ...
    def __post_init__(self):
        ...
        if self.payment_date and not self.formatted_payment_date:
            self.formatted_payment_date = self.payment_date.strftime(
                "%B %d, %Y at %H:%M"
            )


@dataclass
class PaymentFailedEmailContext(BaseEmailContext):
    """Context model for payment failure notifications."""
    ...
    is_admin_alert: Optional[bool] = False
...
@dataclass
class RecurringPaymentEmailContext(BaseEmailContext):
    """Context model for recurring payment notifications."""

    payment_amount: Optional[float] = None
    next_payment_date: Optional[datetime] = None
    formatted_payment_amount: Optional[str] = None
    formatted_next_payment_date: Optional[str] = None
    ...
    def __post_init__(self):
        ...
        if self.payment_amount is not None and not self.formatted_payment_amount:
            self.formatted_payment_amount = f"${self.payment_amount:.2f}"
        if self.next_payment_date and not self.formatted_next_payment_date:
            self.formatted_next_payment_date = self.next_payment_date.strftime(
                "%B %d, %Y"
            )
...
class EmailChannel(BaseNotificationChannel):
    ...
    CONTEXT_MODELS = {
        ...
        NotificationEvents.PAYMENT_FAILED.value: PaymentFailedEmailContext,
        NotificationEvents.PAYMENT_PAUSED.value: RecurringPaymentEmailContext,
        NotificationEvents.PAYMENT_RESUMED.value: RecurringPaymentEmailContext,
        NotificationEvents.PAYMENT_CANCELLED.value: RecurringPaymentEmailContext,
    ...
    def send(
        ...
        try:
            ...
            for recipient in recipients:
                ...
                error_msg = (
                    None if was_sent else "Recipient filtered out or invalid email"
                )
        ...
        for user in recipients:
            try:
                ...
                if settings.get_notification_preference(event.name, "EMAIL"):
                    ...
                    logger.debug(
                        f"User {user.email_address} has disabled email notifications for {event.name}"
                    )

            except Exception as e:
                logger.error(
                    f"Error checking preferences for user {user.email_address}: {str(e)}"
                )
                filtered_recipients.append(user)

        logger.info(
            f"Filtered {len(recipients)} recipients to {len(filtered_recipients)} for event {event.name}"
        )
    ...
    def transform_context(
        ...
        try:
            ...
            logger.error(
                f"Error instantiating context model for event {event.name}: {e}"
            )
            return initial_data
    ...
    def validate_recipients(self, recipients: List["User"]) -> List["User"]:
        ...
        for user in recipients:
            if (
                hasattr(user, "email_address")
                and user.email_address
                and "@" in user.email_address
            ):
                valid_recipients.append(user)
            else:
                logger.warning(
                    f"User {user.id} has an invalid or missing email address"
                )
