"""
Notifications utilities for ClustR application.
Refactored from NotificationManager static methods to pure functions.
"""
...
def send(
    ...
    if not _validate_inputs(event_name, recipients, cluster, context):
        return False
    ...
    if event.bypasses_preferences:
        ...
        return send_sync(event_name, recipients, cluster, context)
...
def send_sync(
    ...
    try:
        return _send_notification_internal(
...
def get_available_channels() -> List[NotificationChannel]:
    ...
    return [
        ...
        if _get_channel_instance(channel) is not None
    ]
...
def send_with_channel_results(
    ...
    return {
        channel.value: _send_via_channel(channel, event, recipients, cluster, context)
...
def _send_notification_internal(
    ...
    def send_channel(channel):
        ...
        try:
            ...
            success = _send_via_channel(channel, event, recipients, cluster, context)
    ...
    return _log_results_and_return_status(
