...
class PaymentErrorHandler:
    """
    Handles payment errors and provides recovery options.
    """
    
    # Error code mappings for different providers
    ...
    def categorize_error(error_message: str, provider: PaymentProvider) -> PaymentErrorType:
        ...
        if any(keyword in error_message_lower for keyword in ['insufficient', 'balance', 'funds']):
        ...
        elif any(keyword in error_message_lower for keyword in ['authentication', 'unauthorized']):
    ...
    def handle_transaction_failure(transaction: Transaction, error_message: str) -> dict[str, Any]:
        ...
        if severity == PaymentErrorSeverity.CRITICAL:
            PaymentErrorHandler._handle_critical_error(transaction, error_type)
    ...
    def handle_recurring_payment_failure(recurring_payment: RecurringPayment, 
        ...
        """
        error_type = PaymentErrorHandler.categorize_error(error_message, PaymentProvider.CASH)
        severity = PaymentErrorHandler.get_error_severity(error_type)
        ...
        if recurring_payment.failed_attempts >= recurring_payment.max_failed_attempts:
            ...
            logger.warning(f"Recurring payment paused due to failures: {recurring_payment.id}")
        
        recurring_payment.save()
        
        # Send notification
        PaymentErrorNotificationManager.send_recurring_payment_failed_notification(
    ...
    def retry_failed_transaction(transaction: Transaction) -> bool:
        ...
        retry_count = transaction.metadata.get('retry_count', 0) if transaction.metadata else 0
...
class PaymentErrorNotificationManager:
    """
    Manager for sending payment error notifications.
    """
    ...
    @staticmethod
    def send_recurring_payment_failed_notification(recurring_payment: RecurringPayment,
