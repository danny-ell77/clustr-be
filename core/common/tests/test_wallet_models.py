...
class BillModelTest(TestCase):
    ...
    def test_bill_payment(self):
        ...
        from core.common.utils.cluster_wallet_utils import CLUSTER_WALLET_USER_ID
        cluster_wallet = Wallet.objects.filter(
            cluster=self.cluster,
            user_id=CLUSTER_WALLET_USER_ID
        ).first()
        
        # Cluster wallet should be created and credited with bill payments
        self.assertIsNotNone(cluster_wallet)
        self.assertEqual(cluster_wallet.balance, Decimal('5000.00'))  # 2000 + 3000
...
class PaymentUtilsTest(TestCase):
    ...
    def test_payment_manager_initialization(self, mock_get_processor):
        ...
        processor = manager.get_processor(PaymentProvider.PAYSTACK)
    ...
    def test_bill_manager_creation(self):
        ...
        with patch('core.common.utils.bill_utils.BillNotificationManager.send_new_bill_notification') as mock_notify:
    ...
    def test_recurring_payment_manager_creation(self):
        ...
        with patch('core.common.utils.recurring_payment_utils.RecurringPaymentNotificationManager.send_setup_confirmation') as mock_notify:
            mock_notify.return_value = True
    ...
    def test_payment_error_handler_categorization(self):
        ...
        from core.common.utils.payment_error_utils import PaymentErrorType
        ...
        self.assertEqual(error_type, PaymentErrorType.INSUFFICIENT_FUNDS)
        ...
        self.assertEqual(error_type, PaymentErrorType.INVALID_CARD)
        ...
        self.assertEqual(error_type, PaymentErrorType.NETWORK_ERROR)
    ...
    def test_payment_error_recovery_options(self):
        ...
        options = PaymentErrorHandler.get_recovery_options(
...
class ClusterWalletTest(TestCase):
    ...
    def test_cluster_wallet_creation(self):
        ...
        wallet = ClusterWalletManager.get_or_create_cluster_wallet(
            self.cluster, 
            str(self.user_id)
        )
    ...
    def test_cluster_wallet_bill_payment_credit(self):
        ...
        from core.common.utils.cluster_wallet_utils import ClusterWalletManager, CLUSTER_WALLET_USER_ID
    ...
    def test_cluster_wallet_analytics(self):
        ...
        # Get analytics
        analytics = ClusterWalletManager.get_cluster_wallet_analytics(self.cluster)
    ...
    def test_cluster_wallet_transfer(self):
        ...
        ClusterWalletManager.add_manual_credit(
            cluster=self.cluster,
            amount=Decimal('5000.00'),
        ...
        balance_info = ClusterWalletManager.get_cluster_wallet_balance(self.cluster)
    ...
    def test_cluster_wallet_insufficient_funds_transfer(self):
        ...
        with self.assertRaises(ValueError) as context:
            ClusterWalletManager.transfer_from_cluster_wallet(
                cluster=self.cluster,
                amount=Decimal('1000.00'),
